import {
  fetchDaddyliveSchedule,
  fetchFootballHighlightEvents,
  fetchfootballLiveStreamEvents,
  fetchRapidFootballeventLink,
  getPPvLandStreams,
} from 'api/streams'
...
const DaddyliveCronjob = Sentry.cron.instrumentCron(
  CronJob,
  'daddyliveCronjobs',
)
export const buildDaddyLiveCatalog = new DaddyliveCronjob(
  '0 1,8,16,20 * * *',
  async () => {
    try {
      ...
      const filtered = await events
        .filter(a => a.type == 'cricket' || a.type == 'soccer')
        .reduce(async (totalEvents: Promise<IDaddyliveEvent[]>, current) => {
            ...
            if (exists?.length > 0) {
              if (current.type == 'cricket') {
                ...
                if (awayTeam) {
                  ...
                  const fixture = cricket.find(
                    a => a.team_a.match(regEx) || a.team_b.match(regEx),
                  )
                  if (fixture) {
                    ...
                    total.push({
                      id: `wwtv-${current.name.replace(/ /gi, '-').toLowerCase()}`,
                      name: current.name,
                      description: current.name,
                      type: current.type,
                      streams: exists.map(a => ({
...
const FetchFixturesCron = Sentry.cron.instrumentCron(
  CronJob,
  'fetchFixtureCronjob',
)
export const fetchFootballFixturesCron = new FetchFixturesCron(
  '00 05 * * *',
  async () => {
    try {
      const footballEvents = await fetchFootballHighlightEvents()
      const events = await footballEvents.reduce(
        async (all: Promise<IFootballEvent[]>, current) => {
          const total = await all
          const poster = await createFootbalPoster({
            homeTeam:
              current.homeTeam.logo ??
              'https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Ftse4.mm.bing.net%2Fth%3Fid%3DOIP.J7c3mMFEqPKkJdxMXNjAqwHaHa%26pid%3DApi&f=1&ipt=e85dcca1a0889f6198b1c6e98144bb1147b4dbe8371c2d4b9d110b53be47a2bd&ipo=images',
            awayTeam:
              current?.awayTeam?.logo ??
              'https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Ftse4.mm.bing.net%2Fth%3Fid%3DOIP.J7c3mMFEqPKkJdxMXNjAqwHaHa%26pid%3DApi&f=1&ipt=e85dcca1a0889f6198b1c6e98144bb1147b4dbe8371c2d4b9d110b53be47a2bd&ipo=images',
            league:
              current?.league?.logo ??
              'https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Ftse4.mm.bing.net%2Fth%3Fid%3DOIP.J7c3mMFEqPKkJdxMXNjAqwHaHa%26pid%3DApi&f=1&ipt=e85dcca1a0889f6198b1c6e98144bb1147b4dbe8371c2d4b9d110b53be47a2bd&ipo=images',
          ...
          total.push({
            awayTeam: current.awayTeam.name ?? 'Away team',
            homeTeam: current.homeTeam.name ?? 'Home team',
            poster,
            id: current.id.toString(),
            league: current?.league?.name ?? 'League',
            name: `${current?.homeTeam?.name} vs ${current?.awayTeam?.name}`,
            time: dayjs.tz(current.date).utc(true).unix(),
            logo:
              current.league.logo ??
              'https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Ftse4.mm.bing.net%2Fth%3Fid%3DOIP.J7c3mMFEqPKkJdxMXNjAqwHaHa%26pid%3DApi&f=1&ipt=e85dcca1a0889f6198b1c6e98144bb1147b4dbe8371c2d4b9d110b53be47a2bd&ipo=images',
          })
...
const EventsApiCronWithCheckIn = Sentry.cron.instrumentCron(
  CronJob,
  'fetchRapidFootballEvents',
)
export const fetchRapidFootballEvents = new EventsApiCronWithCheckIn(
  '*/28 * * * *',
  async () => {
    try {
      await prismaClient.$connect()
      ...
      for (let index = 0; index < events.length; index++) {
        ...
        const date = dayjs
          .tz(`${year}-${month}-${day} ${a.time}`, 'Asia/Yangon')
          .utc()
          .unix()
        ...
        if (event) {
          if (!event.link && a.status == 'Live') {
            const url = await fetchRapidFootballeventLink(
              event.id,
              process.env.RAPID_LIVE_FOOTBALL_API_3!,
            )
            if (url && url != '') {
          ...
          await prismaClient.rapidFootballCatalogue.create({
            data: {
              awayTeam: a.away_name,
              eventId: a.id,
              homeTeam: a.home_name,
              name: `${a.home_name} vs ${a.away_name}`,
              status: a.status,
              date: date,
            },
          })
...
const FetchRapidEventCron = Sentry.cron.instrumentCron(
  CronJob,
  'FetchRapidEventLinkCron',
)
export const fetchRapidEventsLink = new FetchRapidEventCron(
  '*/15 * * * *',
  async () => {
    try {
      await prismaClient.$connect()
      ...
      const missingLinks = events.filter(event => {
        ...
        if (currentTime > endTime) {
          ...
          if (
            ...
            currentTime >= dayjs.unix(event.date).utc().unix()
          ) {
            return true
          } else return false
        }
      ...
      for (let index = 0; index < missingLinks.length; index++) {
        const link = await fetchRapidFootballeventLink(
          missingLinks[index].eventId,
          process.env.RAPID_LIVE_FOOTBALL_API_2!,
        )
        if (link != null && link != '')
          await prismaClient.rapidFootballCatalogue.update({
            where: {
...
const FixtureScheduleCron = Sentry.cron.instrumentCron(
  CronJob,
  'footballScheduleCronjob',
)
export const FootballScheduleCronBuilder = new FixtureScheduleCron(
  '*/5 * * * *',
  async () => {
    try {
      const currentTime = dayjs().utc().unix()
      await prismaClient.$connect()
      const daddyLiveEvent: IDaddyliveEvent[] = (
        (await getFromCache('catalog')) as IDaddyliveEvent[]
      ).filter(a => a.type == 'football' || a.type == 'soccer')
      const footballHighlightEvents: IFootballEvent[] =
        (await getFromCache('football-highlight-events')) ?? []
      const rapidApiEvents = (
        await prismaClient.rapidFootballCatalogue.findMany({
          where: {
            link: {
              isSet: true,
            },
        ...
        async (promise: Promise<IFootballEventCatalog[]>, current) => {
          ...
          try {            
            ...
            const rapidStreams = rapidApiEvents.find(a => {
              return (
                similarity(a.homeTeam?.trim(), current.homeTeam?.trim()) >
                  0.8 ||
                similarity(a.awayTeam?.trim(), current.awayTeam?.trim()) > 0.8
              )
            })
            ...
            if (rapidStreams) {
              streams.push({
                name: current.name,
                externalUrl: rapidStreams.link!,
                title: 'SD',
                url: rapidStreams.link!,
                behaviorHints: { notWebReady: true },
              })
            }
